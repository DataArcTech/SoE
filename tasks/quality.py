...
class QuALITYQuestion(Question):
    ...
    def majority_vote(self, n_samples: int):
        if len(self.attempts)>0:
            ...
            indices = []
            for app in self.attempts:
                indices.append(app['answer_index'])
...
class QuALITYArticle(Document):
    ...
    def get_pii_entities(self, entities):
        ...
        results = []
        entity_analyzer = Entity_Analyzer('en', patterns)
...
class QuALITY(Task):
    ...
    265
    """
    openai_system_check_entities = QUALITY_CHECK_ENTITY_LIST_PROMPT
    ...
    openai_system_generate_two_entity_connection_score = OPENAI_API_SYSTEM_QUALITY_GENERATE_TWO_ENTITIES_CONNECTION_SCORE
        ...
        if split in ['train', 'dev']:
            ...
            if split == 'cur':
                ...
                for doc in self.documents:
                    if doc.title in self.cur_doc_titles:
                        cur_docs.append(doc)
                self.documents = cur_docs
        elif split=='test':
    ...
    def all_questions(self, add_document_context: bool, add_thought_process: bool, sep_after_question: str):
        article_titles = self.cur_doc_titles
        prompts = []
        for document in self.documents:
            if document.title in article_titles:
    ...
    def performance_stats(self):
        ...
        total_documents = len(self.cur_doc_titles)
        val_documents = []
        test_documents = []
        for document in self.documents:
            print(document.title)
            if document.title in self.cur_doc_titles[4*total_documents//5:]:
                val_documents.append(document)
            elif document.title in self.cur_doc_titles:
