...
def data_files2llama_factory_json(data_path, task_type, model_name, file_type="txt"):
    ...
    for file in article_names_all:
        with open(data_path+file, "r", encoding="utf-8") as file:
            if file_type == "txt":
                ...
                result.extend(jload(file)[1:])
    print(len(result))
    for i in range(len(result)):
        text_dict = {}
        text_dict["text"] = result[i]
        result[i] = text_dict
    with open(f'data/encrypt_{task_type}_qwen2.57B.json','w') as f:
        jdump(result, f)

if __name__ == "__main__":
    task_type = 'judge'
    model_name = 'deepseek-chat'
    data_path = f'data/dataset/raw/{task_type}_weighted_edge_entigraph_{model_name}/8/'
    data_files2llama_factory_json(data_path, task_type, model_name, "json")