# ---------------------------------------------------
# + Supporting sentence transformers
# + Entitygraph/Langchain version of RAG
# + https://github.com/ZitongYang/Synthetic_Continued_Pretraining/blob/main/inference/retrieval.py
# ---------------------------------------------------
import sys
import os
...
from utils.prompt_utils import format_name, RAG_QUALITY_PROMPT
from utils.prompt_utils_zh import RAG_JUDGE_PROMPT
...
RETRIEVAL_CONTEXT_CHUNK_PREFIX_ZH = "*上下文 {i}*"
...
"""Embeddings."""
class BGEModel:
    def __init__(self, model_name="BAAI/bge-m3", device = 'cpu', batch_size=16):
        """
        Initialize the BGE model with the pre-trained model and tokenizer.
        """
    ...
    def encode(self, sentences: List[str], show_progress_bar=False):
        ...
        Returns:
            List[List[float]]: A list of embedding vectors for the sentences.
        """
    ...
    def embed_query(self, query: str) -> List[float]:
        ...
        return embeddings[0].tolist()  # Return as a list
...
class SentenceTransformersEmbeddings:
    def __init__(self, model_name: str = "sentence-transformers/all-MiniLM-L6-v2"):
        ...
        Args:
            model_name (str): The name of the SentenceTransformers model to use.
        """
...
def load_langchain_documents_from_dataset(task: QuALITY) -> List[Document]:
    if isinstance(task, QuALITY):
        df = pd.DataFrame(
            data={
    ...
    elif isinstance(task, Judge):
        df = pd.DataFrame(
            data={
                'text': [document.judge_content for document in task.documents],
                'title': [document.title for document in task.documents],
                'year': [document.date for document in task.documents],
                'author': [document.judgeList[0] if len(document.judgeList) else '人员' for document in task.documents]
            }
        )

    # Deduplicate on all columns
    df = df.drop_duplicates()
    logging.info(f"Loaded {len(df)} documents from dataset, after deduplication.")

    return DataFrameLoader(df).load()

def load_langchain_documents_from_judge_dataset(task: Judge) -> List[Document]:
    df = pd.DataFrame(
        data={
            'text': [document.judge_content for document in task.documents],
            'title': [document.title for document in task.documents],
            'date': [document.date for document in task.documents],
            'judgeList': [document.judgeList for document in task.documents]
        })

    # Deduplicate on all columns
    df = df.drop_duplicates()
    logging.info(f"Loaded {len(df)} documents from Judge dataset, after deduplication.")

    return DataFrameLoader(df).load()

def get_metadata_prefix_for_document(document, lang='en'):
...
def split_langchain_documents(
    ...
    for document in documents:
        ...
        metadata_prefix = get_metadata_prefix_for_document(document, lang)
    ...
    if encrypt:
        ...
        if lang=='en':
            ...
            text_path = f'outputs/encrypted_quality-chunk{chunk_size}.txt'
        if lang=='zh':
            text_path = f'outputs/encrypted_judge_all-chunk{chunk_size}.txt'
            anon_lang = 'zh_CN'
        ...
        if text_path and os.path.exists(text_path):
            ...
            for text in texts:
                ...
                encrypted_texts.append(result.text)
...
def get_cached_langchain_embedding_model(
    ...
    # OpenAI Embedding or Sentence Transformer Embeddings
    if 'text-embedding-3' in embedding_model_path:
    ...
    elif 'all-MiniLM' in embedding_model_path or 'Conan-embedding' in embedding_model_path:
        underlying_embeddings = SentenceTransformersEmbeddings(
            model_name=embedding_model_path,
        )
    elif 'bge-m3' in embedding_model_path:
        underlying_embeddings = BGEModel(model_name=embedding_model_path,device='cuda:1')
...
class ChunkReranker:
    def __init__(self, model_name: str = "mixedbread-ai/mxbai-rerank-large-v1", device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        """
        Initialize the Mixedbread Reranker model.
        Args:
    ...
    def rerank(self, query: str, documents: List[str], batch_size: int = 8) -> List[Tuple[str, float]]:
        """
        ...
        Returns:
            List[Tuple[str, float]]: Reranked documents with their relevance scores.
        """
        query_doc_pairs = [(query, doc) for doc in documents]
        scores = []

        # Process query-document pairs in batches
        ...
        reranked_docs_with_scores = list(zip(documents, scores))
...
class LangChainRetriever:
    def __init__(
        ...
        self.docs = load_langchain_documents_from_dataset(task)
        ...
        logging.info(f'Len of splitted text {len(self.texts)}')
        ...
        elif 'mxbai-rerank-large-v1' in self.rerank_model_path:
            self.rerank_model = ChunkReranker(device='cuda')
        elif 'bge-reranker-v2-m3' in self.rerank_model_path:
            self.rerank_model = ChunkReranker(model_name = self.rerank_model_path, device='cuda:1')
        else:
        ...
        if query is None:
            ...
            all_task_questions_for_query_embedding = [query]
    ...
    def rerank_chunks_for_all_queries(
        ...
        for query_str, retrieved_chunks in python_utils.zip_(query_strs, chunks_for_all_queries):
            if query_str in rerank_cached_results.keys():
                ...
                if 'rerank-english-' in rerank_model_path:
                    ...
                    rerank_results = self.rerank_model.rerank(query=query_str,
                        documents=retrieved_chunk_strs)
...
def get_retrieval_prompts(
    ...
    if isinstance(task, QuALITY):
        retrieval_icl_prompt_template = RAG_QUALITY_PROMPT
        chunk_prefix = RETRIEVAL_CONTEXT_CHUNK_PREFIX
        if query is not None:
            retrieval_icl_prompt_template = """
### Instructions
...
"""
    if isinstance(task, Judge):
        retrieval_icl_prompt_template = RAG_JUDGE_PROMPT
        chunk_prefix = RETRIEVAL_CONTEXT_CHUNK_PREFIX_ZH
        if query is not None:
            retrieval_icl_prompt_template = """
### 指令  
...
"""
    if query is None:
        all_task_formatted_questions_and_choices = task.all_questions(
            add_document_context=add_context,
            add_thought_process=True,
            sep_after_question='\n\n'
        )
    else:
        all_task_formatted_questions_and_choices = [query]
    retrieval_prompts = []
...
if __name__ == "__main__":
    ...
    lcr = LangChainRetriever(qual, embedding_model,
                            'recursive', 1024, 0, reranker_path)
    result = lcr.retrieve_chunks_for_all_queries(
        top_k=128,
        rerank_model_path=reranker_path,
    )

    list_of_retrieved_chunks = result.get_rerank_results_for_top_k(
        retrieval_top_k=128,
        rerank_top_k=128,
    )
